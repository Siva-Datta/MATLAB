voices = string({'Morning1.wav','Morning2.wav','Morning3.wav','Lunch1.wav','Lunch2.wav','Lunch3.wav','morning1p.wav','morning2p.wav','morning3p.wav','Lunchp1.wav','Lunchp2.wav','Lunchp3.wav','morning1pj.wav','morning2pj.wav','morning3pj.wav','afternoon1pj.wav','afternoon2pj.wav','afternoon3pj.wav','mayanktest.wav'});%rectangular window
...
for i = 1:length(voices)
    for j = 1:length(voices)
        dist2(i,j) = (sum(log(eng(i,1:No)./eng(j,1:No)).^2))^(0.5);
    end
end
k = 8;
q1o = (sum(dist2(k,2:6)))/6;
q2o = (sum(dist2(k,9:12)))/6 + dist2(k,7)/6;
q3o = (sum(dist2(k,13:18)))/6;
